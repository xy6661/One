...
STYLIZED_DIR = r"/mnt/harddisk2/Zhangmengge/codespace/MyAttn/MyAtt_GSA_contentEnhance_XS/results_total/AdaAttN_test/test_latest/images/"

# VGG预训练权重文件的路径 (请确保此文件存在)
VGG_ENCODER_PATH = '/mnt/harddisk2/Zhangmengge/codespace/MyAttn/MyAttn-main/models/vgg_normalised.pth'
...
def calculate_content_loss(stylized_feats, content_feats):
    ...
    loss_c = nn.MSELoss()(mean_variance_norm(stylized_feats[-1]), mean_variance_norm(content_feats[-1])) + \
             (nn.MSELoss()(mean_variance_norm(stylized_feats[-2]), mean_variance_norm(content_feats[-2]))*188/240)#5/6
