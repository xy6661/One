...
def main():
    ...
    for style_name in os.listdir(style_root):
        ...
        with torch.no_grad():
            # --- 从这里开始，替换原来的 with torch.no_grad(): 代码块 ---
            # ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
            with torch.no_grad():
                # 初始化并加载新的 DilatedMDTA 模块
                # 注意：这里的路径需要根据你训练时保存的模型来决定
                # 通常和 transformer/decoder 在同一个文件夹
                style_attn_3_path = 'attn_adain_video/latest_net_style_attn_3.pth'
                style_attn_4_path = 'attn_adain_video/latest_net_style_attn_4.pth'
                style_attn_5_path = 'attn_adain_video/latest_net_style_attn_5.pth'

                style_attn_3 = DilatedMDTA(dim=256, num_heads=8, bias=True).to(device)
                style_attn_4 = DilatedMDTA(dim=512, num_heads=8, bias=True).to(device)
                style_attn_5 = DilatedMDTA(dim=512, num_heads=8, bias=True).to(device)

                style_attn_3.load_state_dict(torch.load(style_attn_3_path))
                style_attn_4.load_state_dict(torch.load(style_attn_4_path))
                style_attn_5.load_state_dict(torch.load(style_attn_5_path))

                style_attn_3.eval()
                style_attn_4.eval()
                style_attn_5.eval()

                for param in style_attn_3.parameters():
                    param.requires_grad = False
                for param in style_attn_4.parameters():
                    param.requires_grad = False
                for param in style_attn_5.parameters():
                    param.requires_grad = False

                # 对风格图片进行特征提取和处理
                style_feats_raw = encode_with_intermediate(style)

                # 使用 DilatedMDTA 处理风格特征
                style_feats = list(style_feats_raw)
                style_feats[2] = style_attn_3(style_feats_raw[2])
                style_feats[3] = style_attn_4(style_feats_raw[3])
                style_feats[4] = style_attn_5(style_feats_raw[4])
